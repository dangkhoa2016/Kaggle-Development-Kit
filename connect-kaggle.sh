...
make_endpoint_key() {
  ...
  if ! config="$(ssh "${SSH_OPTS[@]}" -G "$SSH_TARGET" 2>/dev/null)"; then
    return 1
  fi
  ...
  [[ -n "$resolved_user" && -n "$resolved_hostname" ]] || return 1
  validate_port "$resolved_port" || return 1
...
prepare_desired_connection() {
  ...
  if ! DESIRED_ENDPOINT="$(make_endpoint_key)"; then
    die "Could not resolve SSH destination for endpoint tracking: $SSH_TARGET"
  fi
...
status_cmd() {
  ...
  if (( STATUS_DESIRED_INVALID )); then
    ...
    DESIRED_ENDPOINT=""
    warn "Could not resolve the desired SSH destination for reporting: $SSH_TARGET"
  fi
