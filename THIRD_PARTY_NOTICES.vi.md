...
Repository không đóng gói Qdrant binary hoặc QNP source checkout. `install/install-qdrant.sh` fetch QNP `1.0.0` tại đúng Git commit `464cb5dbc1117a8a8a6472d76a10c5e329021156` khi chạy. QNP release được pin này dùng MIT License. Sau đó QNP source đã pin tải official Qdrant release binary được chọn vào runtime tree `.system/` đã gitignore.

Qdrant **v1.18.3**, target của release candidate được chọn để validation, dùng Apache License 2.0 trong upstream `qdrant/qdrant`. Các Qdrant version khác mà người dùng có thể chọn giữ license đi kèm đúng upstream release đó và cần được kiểm tra riêng. Public source ZIP của repository này không redistribute QNP hoặc Qdrant.
