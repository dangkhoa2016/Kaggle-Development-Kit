...
case "$ELASTIC_FORCE_RUNTIME_REFRESH" in 0|1) ;; *) die "ELASTIC_FORCE_RUNTIME_REFRESH must be either 0 or 1." ;; esac
[[ "$ELASTIC_HEAP_SIZE" =~ ^[0-9]+[mMgG]$ ]] || die "ELASTIC_HEAP_SIZE must use a form such as 512m or 1g."

REQUESTED_VERSIONS=()
for version in $ELASTIC_VERSIONS; do
    [[ "$version" =~ ^[0-9]+\.[0-9]+\.[0-9]+$ ]] || die "Elastic version '$version' is invalid; use an exact X.Y.Z version."
    [[ " ${REQUESTED_VERSIONS[*]} " == *" $version "* ]] || REQUESTED_VERSIONS+=("$version")
done
[ "${#REQUESTED_VERSIONS[@]}" -gt 0 ] || die "ELASTIC_VERSIONS must not be empty."
list_contains "$ELASTIC_DEFAULT_VERSION" "${REQUESTED_VERSIONS[*]}" || die "ELASTIC_DEFAULT_VERSION must be included in ELASTIC_VERSIONS."
for version in $ELASTIC_AUTO_START_VERSIONS; do
    list_contains "$version" "${REQUESTED_VERSIONS[*]}" || die "ELASTIC_AUTO_START_VERSIONS contains '$version', which is not present in ELASTIC_VERSIONS."
done

COMPONENTS=()
for component in $ELASTIC_COMPONENTS; do
    case "$component" in elasticsearch|kibana|logstash) ;; *) die "Unsupported Elastic component: $component" ;; esac
    [[ " ${COMPONENTS[*]} " == *" $component "* ]] || COMPONENTS+=("$component")
done
[ "${#COMPONENTS[@]}" -gt 0 ] || die "ELASTIC_COMPONENTS must not be empty."
...
install_component() {
    ...
    if [ "$ELASTIC_FORCE_RUNTIME_REFRESH" != "1" ] && [ -x "$target/bin/$component" ]; then
        log "$component $version already exists; skipping download."
        return
    fi
    arch="$(elastic_arch)"
    ...
    [ -n "$expected" ] && [ "$expected" = "$actual" ] || die "SHA512 mismatch for $component $version."

    tar -xzf "$archive" -C "$extracted" --strip-components=1
    [ -x "$extracted/bin/$component" ] || die "The $component $version archive is missing bin/$component."
...
write_version_config() {
    ...
    if list_contains elasticsearch "${COMPONENTS[*]}"; then
        # ES_PATH_CONF replaces Elasticsearch's bundled config directory entirely.
        # Seed the upstream configuration baseline first so required files such as
        # log4j2.properties and jvm.options remain available, then apply the
        # Kaggle Development Kit overrides below.
        runtime_config="$(component_home "$version" elasticsearch)/config"
        [ -d "$runtime_config" ] || die "Missing upstream Elasticsearch config directory: $runtime_config"
        run_root install -d -o "$ELASTIC_SERVICE_USER" -g "$ELASTIC_SERVICE_GROUP" -m 755 "$config/elasticsearch"
        run_root cp -a "$runtime_config/." "$config/elasticsearch/"
...
write_service_helpers() {
    rm -f "$SYSTEM_DIR/elastic-service.sh"
...
HELPER
    ...
    rm -f "$SYSTEM_DIR/elastic-ctl.sh"
