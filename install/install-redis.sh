...
case "$REDIS_FORCE_RUNTIME_REFRESH" in 0|1) ;; *) die "REDIS_FORCE_RUNTIME_REFRESH chỉ nhận 0 hoặc 1." ;; esac
case "$REDIS_BUILD_JOBS" in ''|*[!0-9]*) die "REDIS_BUILD_JOBS phải là số nguyên dương." ;; esac
[ "$REDIS_BUILD_JOBS" -ge 1 ] || die "REDIS_BUILD_JOBS phải >= 1."

REQUESTED_VERSIONS=()
for version in $REDIS_VERSIONS; do
    [[ "$version" =~ ^[0-9]+\.[0-9]+\.[0-9]+$ ]] \
        || die "Redis version '$version' không hợp lệ; dùng exact release dạng X.Y.Z."
    if [[ " ${REQUESTED_VERSIONS[*]} " != *" $version "* ]]; then
        REQUESTED_VERSIONS+=("$version")
    fi
done
[ "${#REQUESTED_VERSIONS[@]}" -gt 0 ] || die "REDIS_VERSIONS không được để trống."
list_contains "$REDIS_DEFAULT_VERSION" "${REQUESTED_VERSIONS[*]}" \
    || die "REDIS_DEFAULT_VERSION=$REDIS_DEFAULT_VERSION phải nằm trong REDIS_VERSIONS."
for version in $REDIS_AUTO_START_VERSIONS; do
    list_contains "$version" "${REQUESTED_VERSIONS[*]}" \
        || die "REDIS_AUTO_START_VERSIONS chứa '$version' nhưng version đó không có trong REDIS_VERSIONS."
done
...
ensure_build_dependencies() {
    log "Đảm bảo build dependencies cho Redis source..."
...
install_version() {
    local version="$1" runtime current stage archive src extracted checksum expected url
    runtime="$(runtime_dir_for "$version")"
    current="$(installed_version "$version" 2>/dev/null || true)"
    if [ "$REDIS_FORCE_RUNTIME_REFRESH" != "1" ] && [ "$current" = "$version" ]; then
        log "Redis $version đã có tại $runtime; bỏ qua build."
        return
    fi
    ...
    if [ -n "$expected" ] && [ "$checksum" != "$expected" ]; then
        die "SHA256 Redis $version không khớp: expected=$expected actual=$checksum"
    fi
    ...
    for bin in redis-server redis-cli; do
        [ -x "$src/src/$bin" ] || die "Build Redis $version thiếu $bin."
...
INFO
    ...
    [ "$current" = "$version" ] || die "Build Redis trả version '$current', expected '$version'."
    ...
    trap - RETURN
    log "Đã cài Redis $version tại $runtime"
}
...
main() {
    ...
    for version in "${REQUESTED_VERSIONS[@]}"; do
        log "Khởi động tạm thời để kiểm tra Redis $version..."
        ...
        [ "$detected" = "$version" ] || die "Redis instance trả version $detected, expected $version."
        ...
        [ "$result" = "ok" ] || die "Redis $version SET/GET test thất bại."
