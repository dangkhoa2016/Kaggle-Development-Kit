...
- Bootstrap giờ an toàn với cold restore: sau khi Kaggle reset VM, nó tái tạo Qdrant service user từ metadata `.system/qdrant/service-user` và sửa lại ownership instance/config đã lưu trong khi vẫn giữ mode chặt `0640` của config.
- Thêm `tests/test-qdrant-cold-restore.sh` bao phủ boundary đó một cách deterministic, cùng GitHub Actions CI lightweight cho manifest, syntax, suite deterministic, SQLite checksum và notebook.
...
- Redis chạy từ bản phát hành upstream pin chính xác: mã nguồn chính thức của từng phiên bản `X.Y.Z` được tải về và biên dịch vào `.system/redis/versions/<version>`.
- Mỗi phiên bản Redis có thư mục instance riêng dưới `.system/redis/instances/<version>`, cho phép nhiều bản pin cùng tồn tại song song.
- Redis chạy dưới system user, không chạy daemon dưới root.
- mise được pin version; installer có thể xác minh installer tải về bằng `MISE_INSTALLER_SHA256` khi cấu hình checksum pin.
