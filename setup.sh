...
# Development runtime root (databases/toolchains). Distinct from the SSH
# STATE_DIR; honors the same project-wide KAGGLE_SYSTEM_DIR contract the
# installers use so auto mode can detect a restored custom root.
DEV_SYSTEM_DIR="${KAGGLE_SYSTEM_DIR:-$PROJECT_ROOT/.system}"
...
ensure_openssh() {
  ...
  # Kaggle already starts with usable Ubuntu package indexes in normal sessions.
  # Try those first. An unconditional `apt-get update` also refreshes unrelated
  # third-party repositories (for example CRAN), so a temporary mirror sync can
  # otherwise block SSH even when openssh-server itself is installable.
  if DEBIAN_FRONTEND=noninteractive apt-get \
      ...
      install -y -qq --no-install-recommends openssh-server; then
    :
  else
    log 'Initial OpenSSH install failed; refreshing APT indexes before one retry...'
    if ! DEBIAN_FRONTEND=noninteractive apt-get \
        -o Acquire::Retries=3 \
        update -qq; then
      log 'WARNING: APT index refresh was incomplete; an unrelated mirror may be syncing. Retrying OpenSSH with the indexes that are available.'
    fi
...
prepare_development_environment() {
  ...
  case "$mode" in
    ...
    bootstrap)
      ...
      ;;
    auto)
      if [[ -d "$DEV_SYSTEM_DIR" ]]; then
        log 'Detected development system root; bootstrapping restored runtime state...'
        bash "$INSTALL_ALL_SCRIPT" bootstrap
