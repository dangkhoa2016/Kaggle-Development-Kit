...
for name in files:
    ...
    for lineno, line in enumerate(text.splitlines(), 1):
        # Preserve the author's exact legal identity in the MIT notice while
        # keeping the broader English-source rule fail-closed everywhere else.
        if name == 'LICENSE' and line == 'Copyright (c) 2026 \u0110\u0103ng Khoa <i.am@dangkhoa.dev>':
            continue
